##########################################
##### Outbreak_Simulations.R #############
##### Simulate outbreaks with an IRT, CRT, and SWT trial
##### Kennedy-Shaffer and Lipsitch #######
##### Update: May 26, 2020 ###############
##### Please Cite https://doi.org/10.1101/2020.05.01.20087429
##### Based on code from Matthew D. Hitchings, available at:
##### https://github.com/mhitchings/Code #
##### For questions, contact Lee Kennedy-Shaffer: lee_kennedyshaffer@g.harvard.edu
##### Approximate Runtime for SWT-A from paper: 1.5 minutes for one simulation of all 3 trial types
##########################################

### Packages Needed: ###

# install.packages("Matrix","Rlab","igraph","deSolve","reshape2","ggplot2","caTools","devtools","survival","coxme","frailtypack","lme4","NetSurv")
require(Matrix)
require(Rlab)
require(igraph)
require(deSolve)
...
nsim <- 100 ### Note: if running more than 1, need to save fullRes_SWT separately for each simulation to analyze later
...
network_epidemic<-function(g,beta,num_introductions,VE,
  ...
  # bCluster - indicator of whether we are running a SWT (2), a cRCT (1) or an iRCT (0)
...
analyse_data <- function(results,trial_nodes,trial_startday,trial_length,ave_inc_period,
  ...
  if (bCluster == 0) {
    ...
    ## Analysis for SWT is conducted externally
    list(NA)
  }
}
...
for (sim in 1:nsim) {
  ...
  fullRes_IRT <- list(results=results, trial_nodes=trial_nodes)
  ...
  fullRes_CRT <- list(results=results, trial_nodes=trial_nodes)
  ...
  numevents_vacc[3,sim] <- sum(trial_infected$TrialStatus==1)
  numevents_cont[3,sim] <- sum(trial_infected$TrialStatus==0)
  ss[3,sim] <- dim(trial_nodes)[1]
  
  NumPds <- ceiling(trial_length/step_interval)
  NumClusts <- length(community_crossovers$Community)
  df <- community_crossovers
  df$indivs <- unlist(lapply(df$Community, function(x) sum(trial_nodes$Community==x)))
  df$StartPd <- (df$DayVaccinated - 56)/step_interval+1
  for (pd in 1:NumPds) {
    StartDay <- 56+(pd-1)*step_interval
    EndDay <- 56+pd*step_interval-1 # Note: to keep period lengths equal, infections on the last day of the trial may be excluded from any period.
    vec <- unlist(lapply(df$Community,
                         function(x) sum(trial_infected$Community[trial_infected$DayInfected >= StartDay & trial_infected$DayInfected <= EndDay]==x)))
    df[[paste0("Pd_",pd)]] <- vec
  }
  cols <- paste0("Pd_",(1:NumPds))
  PdMat <- matrix(rep(1:NumPds,NumClusts),nrow=NumClusts,ncol=NumPds,byrow=TRUE)
  colnames(PdMat) <- cols
  TypeMat <- apply(PdMat, 2, function(x) ifelse(x>df[,"StartPd"],2,
                                                ifelse(x==df[,"StartPd"],1,0)))
  Indivs <- df[,"indivs"]
  OutMat <- apply(df[,cols], 2, function(x) x/Indivs)
  
  fullRes_SWT <- list(results=results, trial_nodes=trial_nodes, crossovers=community_crossovers,
